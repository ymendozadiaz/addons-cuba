...
class CreateInvoiceWzd(models.TransientModel):
    _name = "create.invoice.wzd"
    ...
    ticketcuba_id = fields.Many2one('ticket.cuba', string='ticket', required=True)
    price = fields.Integer(string="Price", related='ticketcuba_id.price')
    invoice_payment_term_id = fields.Many2one('account.payment.term', string='Payment Terms')
    analytic_account_id = fields.Many2one('account.analytic.account', string='Analytic Account')
    description = fields.Char(string='Description', related='ticketcuba_id.description')
    quantity = fields.Float(string='Quantity', default=1.0)
    price_unit = fields.Float(string='Unit Price', default=0.0)
    state = fields.Selection([('new', 'new'), ('invoice', 'Invoice'),('payment','Payment'),('reconcile','Reconcile')], string='State', default='new')
